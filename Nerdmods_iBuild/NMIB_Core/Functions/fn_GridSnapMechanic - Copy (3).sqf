...
/* _WallGridPositions = [];
_WallWorldPositions = [];
for "_a" from -((5*NMIB_PlotSize)+2.5) to ((5*1+2*NMIB_PlotSize)) step (5) do
{
	for "_b" from -((5*NMIB_PlotSize)) to ((5*2*NMIB_PlotSize)) step (5) do
	{
		_WallGridPositions = _WallGridPositions + [[_a,_b,0]];
		_WallWorldPositions = _WallWorldPositions + [ATLToASL (_Plot modelToWorld [_a,_b,0])];
	};
}; */
...
/* 	_NearestWorldPos = [_WallWorldPositions,_CrossHairPASL] call BIS_fnc_nearestPosition;
	...
	_North = [(_NearestGridPos select 0),(_NearestGridPos select 1)+2.5,(_NearestGridPos select 2)];
	_East = [(_NearestGridPos select 0)+2.5,(_NearestGridPos select 1),(_NearestGridPos select 2)];
	_South = [(_NearestGridPos select 0),(_NearestGridPos select 1)-2.5,(_NearestGridPos select 2)];
	_West = [(_NearestGridPos select 0)-2.5,(_NearestGridPos select 1),(_NearestGridPos select 2)];
	_WallGridPositions = [_North,_East,_South,_West];
	...
	_WallWorldPositions = [ATLToASL _NorthWorld,ATLToASL _EastWorld,ATLToASL _SouthWorld,ATLToASL _WestWorld];
	
	_NearestWorldPos = [_WallWorldPositions,_CrossHairPASL] call BIS_fnc_nearestPosition;
	_Id = (_WallWorldPositions find _NearestWorldPos);
	
	if (_Id == 0) then {_Ghost setDir _PlotDir};
	if (_Id == 1) then {_Ghost setDir _PlotDir+90};	
	if (_Id == 2) then {_Ghost setDir _PlotDir+180};	
	if (_Id == 3) then {_Ghost setDir _PlotDir-90};	
} else {
	_NearestWorldPos = [_FloorWorldPositions,_CrossHairPASL] call BIS_fnc_nearestPosition;
	_NearestGridPos = _FloorGridPositions select (_FloorWorldPositions find _NearestWorldPos);
};
